...
public class ForoServiceImpl extends BaseService implements ForoService {
     ...
     */
    @Override
    public List<InfoDiscussionsDTO> getInfoDiscussions(FilterDiscussionDTO fdfto) throws RuntimeException {
        try {
            DiscussionJpaController discussionJpaController = (DiscussionJpaController) getContextAttribute("discussionJpaController");
            List<Discussion> listDiscussions = discussionJpaController.getInfoDiscussionsByFilters(Integer.parseInt(fdfto.getIdModule()), fdfto.getDateTime(), fdfto.getSearchFilter());
            if (listDiscussions != null && !listDiscussions.isEmpty()) {
                ...
                for (Discussion discussion : listDiscussions) {
                    ...
                    discussionDTO.setIdModule(Integer.parseInt(fdfto.getIdModule()));
     ...
     */
    @Override
    public List<InfoCommentsDTO> getInfoComments(FilterCommentsDTO commentsDTO) throws RuntimeException {
        try {
            CommentsJpaController commentsJpaController = (CommentsJpaController) getContextAttribute("commentsJpaController");
            List<Comments> listComments = commentsJpaController.getInfoCommentsByFilters(Integer.parseInt(commentsDTO.getIdDiscussion()), commentsDTO.getDateTime(), commentsDTO.getSearchFilter());
